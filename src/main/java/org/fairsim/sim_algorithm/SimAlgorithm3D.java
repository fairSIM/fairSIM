...
public class SimAlgorithm3D {
	 ...
	 * @param visualFeedback The amount of visual feedback, currently not overly
	 *                       used
	 * @param wienParam      The Wiener parameter
	 * @param fitLevel       Which parts of the fit to run
	 * @param runFastFit     If to run the sub-pixel fit on projected data (much
	 *                       faster)
	 *
	 */
			...
			for (int angIdx = 0; angIdx < param.nrDir(); angIdx++) {
				...
				if (visualFeedback > 0) {
					...
					for (int i = 0; i < par.nrBand(); i++) {
						...
						Tool.trace("REC: Dir " + angIdx + " band: " + i + " Wiener denominator visualization wf: "
								+ wienParam);
						for (int z = 0; z < d; z++) {
							Vec2d.Real tmp = Vec2d.createReal(2 * w, 2 * h);
							tmp.slice(denom, z);
