...
public class Correlation3d {
	 ...
	 * 
	 * @param band0            Low freq band to correlate to (that does not change
	 *                         position)
	 ...
	 * @param weightLimit      If > 0, consider only freq where both weights are
	 *                         over this limit
	 * @param search           kx,ky will be varied +-search
	 * @param projectForSearch if a 2D projection is used for the sub-pixel search
	 * @param cntrl            control vector (size 30x10) to be filled with power
	 *                         spectra, may be null
	 */
	public static double[] fitPeak(Vec3d.Cplx band0, Vec3d.Cplx band1,
		...
		for (int iter = 0; iter < 3; iter++) {
			 ...
			 * Tool.trace(String.format("3D full: Correlation z %7.3f %8.5f",
			 * zpos,zCorr.hypot()));
			 * 
			...
			if (!projectForSearch) {
				...
				final Cplx.Double scal2d = new Cplx.Double(1. / b0proj.norm2()); // for corr. normalization
	 ...
	 * (Also, a minimum distance of 0.15*hypot(kx,ky) is ensured to the bands
	 * centers).
	 ...
	 * are set to zero in both bands. All others are divided by weight (if switch is
	 * set).
	 * 
	 * @param band0       The lower band, will not be moved
	 * @param band1       The higher band, shifted to kx,ky
	 ...
	 * @param kx          x-coordinate of shift
	 * @param ky          y-coordinate of shift
	 * @param dist        Minimal distance (as fraction of hypot(kx,ky)) to band
	 *                    centers (std use 0.15)
	 * @param weightLimit Components where either OTF is below this limit are set to
	 *                    0
	 * @param divideByOtf If set, all components over weightLimit are divided by OTF
	 *                    weight
	 */
		...
		for (int z = 0; z < d; z++)
			for (int y = 0; y < h; y++)
				for (int x = 0; x < w; x++) {
					...
					/*
					 * // TODO: the 'z>2' avoids all axial contribution (fine for band 0<>2, but has
					 * to be fixed!)
