...
public class SimAlgorithm {
	 ...
	 * @param fitExclude     How much (in fraction of OTF support) to exclude from
	 *                       fit
	 * @param idf            ImageDisplayFactory for intermediate output (may be
	 *                       null)
	 ...
	 * @param inFFT             The input images (in Fourier space)
	 * @param fitBand           On which band to perform the kx,ky fit
	 * @param fitExclude        How much (in fraction of OTF support) to exclude
	 *                          from fit, for each orientation
	 * @param idf               ImageDisplayFactory for intermediate output (may be
	 *                          null)
	 * @param visualFeedback    Feedback Amount of visual feedback, 0..4
	 * @param tEst              Runtime measurement (may be null)
	 * @param keepPhases        If true, phase information from SimParam will be
	 *                          used in band separation
	 * @param contextForLogging Context information for log output only (e.g. which
	 *                          dataset is being processed), can be null
	 */
	public static void estimateParameters(final SimParam param,
			Vec2d.Cplx[][] inFFT, final int fitBand, final double[] fitExclude,
			final ImageOutputFactory idf,
		...
		if (fitBand != 1 && fitBand != 2)
			throw new RuntimeException(
					"Fitband neither 1 nor 2");

		if (fitExclude.length != param.nrDir())
			throw new RuntimeException(
		...
		for (int angIdx = 0; angIdx < param.nrDir(); angIdx++) {
			...
			if (lb != hb) {
				...
				// At the peak position found, extract phase and modulation from band0 <-> band
				// 1
				...
				Tool.trace(String.format(
						"--> Physical pattern: dir (%d) cycle/micron %6.4f angle(deg) %6.4f phase(deg) %6.4f [%s]",
						angIdx,
			...
			if (visualFeedback > 0) {
				...
				ImageDisplay.Marker excludedDC = new ImageDisplay.Marker(w / 2, h / 2, minDist * 2, minDist * 2, true);
	 ...
	 * This follows the non-iterative phase estimation based on auto-correlation by
	 * Wicker et al.
	 * It requires(!) the k-Vectors to be set correctly (usually doable by running
	 * the parameter
	 ...
	 * @param param                   The SIM parameter instance to work on
	 * @param inFFT                   The input images (in Fourier space)
	 * @param idf                     ImageDisplayFactory for intermediate output
	 *                                (may be null)
	 * @param visualFeedback          Feedback Amount of visual feedback, -1 (off)
	 *                                ..4
	 * @param otfBeforeShift          Apply the OTF before shifting bands
	 * @param imgClipScale            Clip zero values and scale (0..255) output
	 *                                images?
	 * @param tRec                    Runtime measurement (may be null)
	 * @param widefieldResult         The widefield image computed during the
	 *                                reconstruction (may be null)
