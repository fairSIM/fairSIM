...
public class OtfCreator {
	...
	public Vec2d.Cplx[] createOtf(Vec3d.Cplx[][] inputImgs, int visualFeedback) {
		...
		// TODO: move the DisplayWrapper to an interface, so the dependency to Fiji is
		// not needed here
		ImageStackOutput iso1 = new DisplayWrapper5D(width, height, depth, nrAngle, nrBands, "band-sep input data");
		// ImageStackOutput iso2 = new DisplayWrapper5D( width, height, depth,
		// 2,nrBands*2,"full OTFs");
		...
		for (int ang = 0; ang < nrAngle; ang++) {
			...
			ImageStackOutput isoMag = new DisplayWrapper5D(width, height, depth, 4, 3,
					String.format("ang %d: OTF MAG full comp.", ang));
			ImageStackOutput isoPha = new DisplayWrapper5D(width, height, depth, 4, 3,
