...
public class SimParam implements Vec2d.Size, Vec3d.Size {
	...
	private CLIPSCALE clipScaleMode = CLIPSCALE.NONE; // clip&scale of output

	private FilterStyle filterStyle = FilterStyle.Wiener; // which filter to use
	...
	public class Dir {
		...
		public double[] getModulations() {
			...
			for (int b = 0; b < nrBands; b++) {
				if (ret[b] < modLowLimit)
					ret[b] = defaultModulation;
				if (ret[b] > modHighLimit)
					ret[b] = defaultModulation;
			}
			return ret;
		}
	...
	public void saveConfig(Conf.Folder cfg) {
		...
		for (int d = 0; d < nrDirs; d++) {
			...
			df.newDbl("shift").setVal(new double[] { dir.pX * (dir.nrBands - 1), dir.pY * (dir.nrBands - 1) });
