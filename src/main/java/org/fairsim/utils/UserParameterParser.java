...
public class UserParameterParser {
	...
	/**
	 * Register the static members of a class. This will fail with a runtime
	 * exception should this class
			...
			if (up != null) {
				...
				if (!(a.hasInstance || preregister)) {
					throw new RuntimeException(
							"non-static user-settable variables from a static context: " + fld.getName());
				}
	 ...
	 * in the given class to be static. Typical use case are scripts run from the
	 * command line.
	 *
	 ...
	 *
	 * Note: if one of the arguments is 'help', '-h' or '--help' an error (-1) is
	 * returned.
	 * A help message will be printed (if verbosity is not -1) and the programm will
	 * exit with error code 1 (if exitOnFail is set).
	 *
	 * @param arg              The String [] arg of the main method
	 * @param obj              The object to modifiy the argument of (set to null
	 *                         for static version)
	 * @param cls              The class to modify the arguments of (only used when
	 *                         obj==null)
	 * @param failOnNonMatched If to fail if parameters could not be parsed (typos,
	 *                         etc.)
	 * @param exitOnFail       If to call System.exit() if parameters could not be
	 *                         parsed
	...
	public static int defaultParser(String[] arg, Object obj, Class cls,
		...
		for (String i : arg) {
			...
			if (_upp.parseParameter(a[0], a[1])) {
				...
				// check if the parameter is preregistered (so it might just not be found in the
				// currently parsed set)
