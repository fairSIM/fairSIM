...
public class Args {
	...
	public void printParams() {
		...
		for (ArgEntry i : entries.values()) {
			...
			String val = "";
			if (i.type == ArgType.INT)
				val = String.format("%12d", i.intValue);
			if (i.type == ArgType.DBL)
				val = String.format("%12.5f", i.dblValue);
			if (i.type == ArgType.STR)
				val = String.format("%12s", i.strValue);
