...
public class ImageControl {
	...
	final Tiles.LComboBox<String> videoAutoUpdateMode = new Tiles.LComboBox<String>("auto-update", videoUpdateModes);
	final Tiles.LComboBox<String> batchUpdateMode = new Tiles.LComboBox<String>("batch-update", batchUpdateModes);
	...
	final Tiles.LComboBox<String> prefactorMethodBox = new Tiles.LComboBox<String>("estimate, by", "average", "median");
	...
	public ImageControl(final JFrame baseframe, ImageSelector is,
		...
		zSliceVideoSpinner = new Tiles.LNSpinner("zSlices", 1, 1, 200, 1);

		ImageSelector.ImageInfo prototypeImage = new ImageSelector.ImageInfo(512, 512, 15, 1, 1, 0.082, 0.125,
				"some really long image filename as these will often be used", -1);

		imgBox.box.setPrototypeDisplayValue(prototypeImage);
		...
		showCorrectedImagesButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				ImageDisplay correctedDataDisplay = idpFactory.create(theImages[0][0].vectorWidth(),
						theImages[0][0].vectorHeight(),
	...
	void updateIfVideoCanBeUsed() {
		...
		if ((inf.nrSlices * inf.nrTimepoints) % (zplSel * simParam.getImgPerZ()) != 0) {
			...
			if (isVideoStack) {
				...
				totalTimePoints = (autoVid) ? (inf.nrTimepoints)
						: ((inf.nrTimepoints * inf.nrSlices) / simParam.getImgPerZ() / zplSel);
				...
				videoAutoUpdateMode.setEnabled(false);
				videoAutoUpdateMode.setSelectedIndex(0);
				maxTimePointsLabel.setText(
						"(t-max: " + ((inf.nrTimepoints * inf.nrSlices) / simParam.getImgPerZ() / zplSel) + ")");
				totalTimePoints = 1;
			}
	...
	class AutoUpdateThread extends Thread {
		...
		public void run() {
			...
			simPreviewDisplay = idpFactory.create(simWidth, simHeight, "SIM preview");
	...
	void importImageDialog(final ImageSelector.ImageInfo img,
		...
		if (img.nrSlices > simParam.getImgPerZ()) {
			...
			Integer[] n = new Integer[img.nrSlices / simParam.getImgPerZ()];
		...
		Tiles.LNSpinner pxlSizeSpinner = new Tiles.LNSpinner("nm", pxlSize, 10, 500, 0.1);
	...
	void openBatchDialog() {
		...
		final Tiles.LNSpinner startSpinner = new Tiles.LNSpinner("start", 1, 1, totalTimePoints, 1);
		final Tiles.LNSpinner stopSpinner = new Tiles.LNSpinner("stop", totalTimePoints, 1, totalTimePoints, 1);
	...
	class BatchReconstructionThread extends Thread {
		...
		public void run() {
			...
			ImageDisplay simOutputDisplay = idpFactory.create(simWidth, simHeight, "SIM batch results");

			ImageDisplay widefieldOutputDisplay = (compWidefield)
					? (idpFactory.create(simWidth, simHeight, "widefield batch results"))
					: (null);

			ImageDisplay filteredWidefieldOutputDisplay = (compFilteredWidefield)
					? (idpFactory.create(simWidth, simHeight, "filtered widefield batch results"))
					: (null);
			...
			for (int timePos = start; timePos < stop; timePos++) {
				...
				if (updateMode >= 2) {
					Tool.trace(
							String.format("Batch mode: Running individual absolute phases (time slice %d)", timePos));
					SimAlgorithm.estimateAbsolutePhases(
							simParam, theFFTImages, null);
				}

				Vec2d.Real widefield = (compWidefield) ? (Vec2d.createReal(simWidth, simHeight)) : (null);
				Vec2d.Real filteredWidefield = (compFilteredWidefield) ? (Vec2d.createReal(simWidth, simHeight))
						: (null);
