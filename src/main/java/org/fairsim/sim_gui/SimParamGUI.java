...
class SimParamGUI implements TableModel {
	...
	private List<TableModelListener> tableListen = new ArrayList<TableModelListener>();
	...
	void displayDialog() {
		...
		Tiles.LComboBox<String> degOrRad = new Tiles.LComboBox<String>("Angle and phase in", "rad", "deg");
		...
		Tiles.LComboBox<String> polOrCart = new Tiles.LComboBox<String>("Shift vector as", "Angle/Length (pol.)",
				"Shift px,py (cart.)");
		...
		Tiles.LComboBox<String> pxlOrMicrons = new Tiles.LComboBox<String>("Shift vector length",
	...
	public Double getValueAt(int row, int col) {
		...
		switch (col) {
			...
			case 1: // either full kx len or py
				double tmplen = (displayLenAng) ? (par.dir(row).getPxPyLen(hBand)) : (par.dir(row).py(hBand));
				switch (vectorLengthUnit) {
					case 0: // raw pxl
						return tmplen;
					case 1: // cycles / microns
