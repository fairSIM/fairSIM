...
public class PlainImageDisplay {
	...
	public void refresh() {
		...
		for (int y = 0; y < miniMap.our_height; y++) {
			for (int x = 0; x < miniMap.our_width; x++) {
				...
				int posImg = (x * imageWidth / miniMap.our_width + (y * imageHeight / miniMap.our_height) * imageWidth)
						* 3;
	...
	static {
		for (int i = 0; i < gammaSRGB.length; i++) {
			...
			// System.out.println(String.format("%d %7.5f %d #srgb-gamma", i, pos,
			// gammaSRGB[i]));
		}
	}
		...
		public int getByteCount() {
			switch (this) {
				...
				case FLOAT_32:
					return 4;
				default:
					throw new IllegalArgumentException("Unknown pixel type: " + this);
			}
		}
	}
	...
	public PlainImageDisplay(int nrChannels, int w, int h, boolean slidersOnTop,
		...
		for (int ch = 0; ch < nrChannels; ch++) {
			...
			final Tiles.LComboBox<Integer> bitDepthSelector = new Tiles.LComboBox<Integer>("bits", 8, 10, 12, 14, 16);
			...
			Tiles.LComboBox<LUT> lutSelector = new Tiles.LComboBox<LUT>("LUT", LUT.values());
		...
		Tiles.LComboBox<CROSSHAIRS> crosshairSelector = new Tiles.LComboBox<CROSSHAIRS>("Crosshair",
				CROSSHAIRS.values());
		...
		Tiles.LComboBox<Tiles.NAMED_COLOR> crosshairColorSelector = new Tiles.LComboBox<Tiles.NAMED_COLOR>("color",
				Tiles.NAMED_COLOR.values());
	...
	static class ImageComponent extends JComponent {
			...
			MouseAdapter mouseAdapter = new MouseAdapter() {
				...
				public void mouseWheelMoved(MouseWheelEvent e) {
					...
					if (e.getWheelRotation() < 0) {
						if (zoomLevel < 8) {
							...
							if (viewportY + (viewportHeight / zoomLevel) > imageHeight)
								viewportY = imageHeight - (viewportHeight / zoomLevel);
							if (viewportX < 0)
								viewportX = 0;
							if (viewportY < 0)
								viewportY = 0;
				...
				public void mousePressed(MouseEvent e) {
					...
					if (leftButton) {
						...
						// System.out.println("Mouse pressed: "+x+" "+y+" left button state
						// "+leftButton);
					}
				}
				...
				public void mouseDragged(MouseEvent e) {
					...
					if (ourUpdateListener != null) {
						// update the viewport
						ourUpdateListener.newZoomOrPosition(zoomLevel, viewportX, viewportY, viewportWidth,
								viewportHeight);
					}
					paintImage();
				}
		...
		public void setImage(int ch, ByteBuffer img, PXL_TYPE bytePerPixel) {
			...
			if (img.remaining() < imgSize)
				throw new IllegalArgumentException(
						"Insufficient data in ByteBuffer. Expected: " + imgSize + ", Available: " + img.remaining());
					...
					for (int x = 0; x < imageWidth; x++) {
						...
						for (int ch = 0; ch < nrChannels; ch++) {
							if (show[ch]) {
								...
								if (val > currentImgMax[ch])
									currentImgMax[ch] = (int) val;
								if (val < currentImgMin[ch])
									currentImgMin[ch] = (int) val;
						...
						if (displayInSRGB) {
							...
							for (int col = 0; col < 3; col++) {
								if ((linRGB[col]) >= 1.0f)
									linRGB[col] = 1 - 0.5f / gammaSRGB.length;
								if ((linRGB[col]) < 0.0f)
									linRGB[col] = 0.0f;
							}
							...
							for (int i = 0; i < 3; i++) {
								imgDataBufferSRGB[3 * (y * imageWidth + x)
										+ i] = gammaSRGB[(int) (linRGB[i] * gammaSRGB.length)];
							}
						} else {
							// use the input directly as sRGB pixels
							for (int i = 0; i < 3; i++) {
								imgDataBufferSRGB[3 * (y * imageWidth + x) + i] = (byte) (cieXYZ[i] * 255); // scale to
																											// 0-255
							}
						}
					}
					...
					for (int x = 0; x < viewportWidth; x++) {
						...
						for (int i = 0; i < 3; i++) {
							if (xPos < 0 || xPos >= imageWidth || yPos < 0 || yPos >= imageHeight) {
								...
								imgDataOnScreen[3 * (x + y * viewportWidth)
										+ i] = imgDataBufferSRGB[3 * (xPos + yPos * imageWidth) + i];
							}
						}
					}
			}
		...
		public void paint(Graphics g) {
			...
			if (crosshair > 0) {
				...
				if (crosshair % 2 == 0) {
					Line2D line1 = new Line2D.Double(
							viewportWidth / 10., viewportHeight / 10., viewportWidth * 9. / 10.,
							viewportHeight * 9. / 10.);
					Line2D line2 = new Line2D.Double(
							viewportWidth * 9 / 10., viewportHeight / 10., viewportWidth / 10.,
							viewportHeight * 9. / 10.);
					((Graphics2D) g).draw(line1);
					((Graphics2D) g).draw(line2);
				}
		...
		public void setZoom(int level, int xPos, int yPos) {
			...
			// System.out.println("Updated ROI: "+xPos+" "+yPos+"/"+zoomX+" "+zoomY+"
			// l:"+zoomLevel);
			if (ourUpdateListener != null)
				ourUpdateListener.newZoomOrPosition(zoomLevel, viewportX, viewportY, viewportWidth, viewportHeight);
		}
				...
				public void at(int i) {
					...
					for (int y = 0; y < height; y++)
						for (int x = 0; x < width; x++) {
							...
							} else if ((x > size - 200 && x < size - 150) || (y > size - 300 && y < size - 250)) {
