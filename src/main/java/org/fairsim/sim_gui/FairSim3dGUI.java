...
public class FairSim3dGUI {
	...
	// presets for the Wiener value spinner (only min/max, default values, see
	// below!)
	final double wienerLowest = 0.0001, wienerHighest = 0.2, wienerSteps = 0.00025;

	// presets for the Apo filter spinners (only min/max, default values, see
	// below!)
	...
	public FairSim3dGUI(DefineMachineGui dmgIn, ImageSelector.ImageInfo imgs, ImageSelector imgSrc,
		...
		// TODO: once they are actually used in the reconstruction, re-enable them in
		// the GUI
		...
		if (autostart) {
			...
			startReconstructionThread(headless, resultImageFile, -2); // Todo: propagate visual feedback level in
																		// autostart mode
	...
	class ChannelPanel {
		...
		ChannelPanel(int ch, String presets) {
				...
				for (int i = 0; i < tokens.length; i++) {
					...
					if (tok.startsWith("p")) {
						...
						if (chIdx < dmg.channels.size()) {
							Tool.trace("ch" + (chNr + 1) + ": using presets #" + chIdx + " ("
									+ dmg.channels.get(chIdx).toString() + ")");
						} else {
					...
					if (tok.startsWith("apoL")) {
						...
						if (apoPreset >= apoLateralLowest && apoPreset <= apoLateralHighest) {
							...
							Tool.error("ch" + (chNr + 1) + ": Apo lateral preset out of range " + apoLateralPreset,
									false);
						}
					}
					...
					if (tok.startsWith("apoType")) {
						if (tok.length() > 7 && tok.charAt(7) == '=') {
							...
							if (atype.equals("linear")) {
								...
								Tool.error("ch" + (chNr + 1) + ": No match for Apo type preset: linear, cosine, off",
										false);
							}
						} else {
							Tool.error("ch" + (chNr + 1) + ": Specify Apo type preset: apoType=[linear, cosine, off]",
									false);
						}
					}
			...
			// we can check the OTF only after everything has parsed, as only now we know
			// which channel preset to use
			if (otfIdx != 0) {
			...
			DefineMachineGui.ChannelTab[] channelsAvailable = new DefineMachineGui.ChannelTab[dmg.channels.size()];
