...
public class ParameterControl {
	...
	private boolean fitIndividualPhaseEstimate = false; // if to run individual (absolute) phase estimates for each raw
														// frame
	...
	void showParameterResults() {
		...
		for (int ang = 0; ang < simParam.nrDir(); ang++) {
			...
			int nr = ((mod >= .6995) ? (0)
					: ((mod >= .4995) ? (1) : ((mod >= 0.2995) ? (2) : ((mod >= .0995) ? (3) : (4)))));

			String tab = String.format("<tr><td> %4.2f </td><td> %5.3f </td><td>" +
					"<font %s >%s</td></tr>",
		...
		htmlContent += "<li><font " + assesment[0][0]
				+ ">good values (0.7 to 1.0)</font> point to a well-aligned system with high SNR</li>";
		htmlContent += "<li><font " + assesment[1][0]
				+ ">usable values (0.5 to 0.7)</font> point to medium SNR or some disalignment<br />";
		htmlContent += "<li><font " + assesment[2][0]
				+ ">weak values (0.3 to 0.5)</font> point to low SNR or alignment problems<br />";
	...
	void configureEstimation() {
		...
		final Tiles.LComboBox<String> verbosityBox = new Tiles.LComboBox<String>("Intermediate results", verbosity);
		...
		final Tiles.LComboBox<Integer> fitBandBox = new Tiles.LComboBox<Integer>("Band to use for shift vector est.",
				bands);
		...
		for (int d = 0; d < simParam.nrDir(); d++) {
			...
			Tiles.LComboBox<Double> box = new Tiles.LComboBox<Double>(
					String.format("Freq. exclude from fit, angle %d", d), excludes);
			...
			// check for the entry in exlcudes[] array that is closest to the current
			// fitExclude value
			int closestIndex = 5;
			{
		...
		// Action listener on first box, that updates all others if separateExclBox is
		// not selected
		...
		final Tiles.LComboBox<Integer> phaseStepBox = new Tiles.LComboBox<Integer>("phase step",
				new Integer[] { 1, 2, 3, 4 });
