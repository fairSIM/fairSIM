 ...
 * This is not meant to be run unmodified, but as a working example starting
 * point
...
public class StepByStep implements PlugIn {
	...
	static public Vec2d.Real stepByStepReconstruction(short[][] inputImages) {
		...
		// 2 - create a 'SimParam' instance that will hold all the reconstruction
		// parameters
		...
		for (int ang = 0; ang < simParam.nrDir(); ang++) {
			for (int pha = 0; pha < simParam.nrPha(); pha++) {
				...
				// subtract background (this clips to 0, and returns percentage of clipped
				// pixels)
		...
		// which band to fit to (on three-beam data typically: 1 - more robust, 2 - more
		// precise)
		final int fitBand = 2;

		// if positive: region to exclude from fit, in relation OTF cutoff freq.
		// if negative: only run a fine estimate of kx,ky [within +-2px] using the
		// values stored in SimParam as starting point
		final double fitExclude = (doNotTryToFindK0) ? (-1) : (0.6);

		// how much feedback / output the reconstruction process generated. Only useful
		// if it also
		// has an 'ImageFactory' to write these results into.
		final int visualFeedback = -1;

		// we need some default values (these are for the OMX U2OS example set) if we
		// don't run the coarse estimation
		if (doNotTryToFindK0) {
		...
		// clip the output to [0..255] (clip&scale), to [0..max] (clip) or leave neg.
		// values in (none)
