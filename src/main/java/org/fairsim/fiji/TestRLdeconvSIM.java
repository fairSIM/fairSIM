...
public class TestRLdeconvSIM implements PlugIn {
	...
	public void runReconstruction(ImageStack inSt) {
		...
		otfPr = OtfProvider.fromEstimate(otfNA, emWavelen, otfCorr);

		// Reconstruction parameters: #bands, #directions, #phases, size, microns/pxl,
		// the OTF
		final SimParam param = SimParam.create(nrBands, nrDirs, nrPhases, w, pxSize, otfPr);
		...
		// (used for reconstruction, or as starting guess if 'locatePeak' is off, but
		// 'findPeak' on)
		...
		if (findPeak) {
			...
			for (int angIdx = 0; angIdx < param.nrDir(); angIdx++) {
				...
				if (visualFeedback > 0) {
					...
					ImageDisplay.Marker excludedDC = new ImageDisplay.Marker(w / 2, h / 2, minDist * 2, minDist * 2,
							true);
