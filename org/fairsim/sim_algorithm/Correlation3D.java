...
/** Sim parameter estimations through correlations, for 3D input data. */
public class Correlation3D {
     ...
     * */
    public static double [] fitPeak( Vec3d.Cplx band0_3d, Vec3d.Cplx band1_3d,
	...
	for ( int iter=0; iter<3; iter++ ) {
	    ...
	    // copy input data
	    final Vec3d.Cplx b0_3d = band0_3d.duplicate();
	    final Vec3d.Cplx b1_3d = band1_3d.duplicate();

	    // define common region, with current search guess
	    commonRegion( b0_3d, b1_3d, bn0, bn1, otf, kx, ky, 0.15, weightLimit, true);
	   
	    final Vec2d.Cplx b0 = b0_3d.project(); 
	    final Vec2d.Cplx b1 = b1_3d.project();
	    ...
	    Tool.trace(String.format("Search %2d: kx [%6.3f -- %6.3f] ky [%6.3f -- %6.3f]",
	    ...
	    // set new guess
	    kx=newKx; ky=newKy;    
	    Tool.trace(String.format("New kx,ky now: %6.3f %6.3f (%6.3f) pha %6.3f mag %5.4e",
		kx,ky,Math.hypot(ky,kx), resPhase, resMag));
	    search/=5;
	}

	t1.stop();
	Tool.trace("... correlation done: "+t1);
	...
	Correlation.commonRegion( b0, b1, bn0, bn1, otf, kx,ky, 0.15, weightLimit, true );
	...
	for (int x=0; x<w; x++) {
	    ...
	    double rad = Math.sqrt( 
		((x<w/2)?(x*x):((x-w)*(x-w))) + ((y<h/2)?(y*y):((y-h)*(y-h)))  );
	    double radAx = Math.sqrt( (z<d/2)?(z*z):((z-d)*(z-d)));
